...
"$WARDEN" check demo/before --rules "$RULES"; echo "exit: $?  (1 = blocked)"

rule "3. BEFORE — same run as a decision record (JSON)"
"$WARDEN" check demo/before --rules "$RULES" --format json

rule "4. AFTER — every violation fixed"
"$WARDEN" check demo/after --rules "$RULES"; echo "exit: $?  (0 = pass)"
